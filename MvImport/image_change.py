...
#sys.path.append("D:/Work_app/MVS海康/MVS/Development/Samples/Python/MvImport")
...
if platform.system() == "Windows":
    ...
    # 在非 Windows 系统上不需要 WINFUNCTYPE
    pass


# 初始化全局变量
lower_blue = [111, 163, 19]
upper_blue = [140, 255, 255]

#轮廓1和2
#分别为第一个和第二个轮廓的最低点和最高点
contour_H_L = [[[0, 0], [0, 0]], [[0, 0], [0, 0]]]

def update_thresholds(x):
    # 更新阈值
    lower_blue[0] = cv2.getTrackbarPos('Lower Hue', 'Threshold Adjustments')
    lower_blue[1] = cv2.getTrackbarPos('Lower Sat', 'Threshold Adjustments')
    lower_blue[2] = cv2.getTrackbarPos('Lower Val', 'Threshold Adjustments')
    upper_blue[0] = cv2.getTrackbarPos('Upper Hue', 'Threshold Adjustments')
    upper_blue[1] = cv2.getTrackbarPos('Upper Sat', 'Threshold Adjustments')
    upper_blue[2] = cv2.getTrackbarPos('Upper Val', 'Threshold Adjustments')
...
class OutFrame(ctypes.Structure):
...
def openCV_dealwith(image_data , color):
    if color == 1 :
        # 将图像转换为HSV颜色空间
        ...
        # 创建蓝色区域的掩膜
        mask = cv2.inRange(hsv_image, np.array(lower_blue), np.array(upper_blue))
        ...
        for i, contour in enumerate(contours):
            # 确保轮廓是有效的 numpy 数组
            if contour is not None and len(contour) > 2:  # 至少需要3个点才能形成有效轮廓
                try:
                    area = cv2.contourArea(contour)
                    areas.append((i, area))
                except cv2.error as e:
                    print(f"计算轮廓 {i} 面积时出错: {e}")
                    continue
            else:
                print(f"轮廓 {i} 无效，跳过")

        # 按面积排序，找到最大的两个轮廓
        largest_contours = sorted(areas, key=lambda x: x[1], reverse=True)[:2]
        ...
        for i, contour in enumerate(contours):
            ...
            for point in contour:
                ...
                if y > max_y:
                    max_y = y 
                    max_x = point[0][0]
            print(f"  最低点坐标: {min_x}, {min_y}")
            print(f"  最高点坐标：{max_x}, {max_y}")
            if i < 2 :
                contour_H_L[i][0][0] = min_x
                contour_H_L[i][0][1] = min_y
                contour_H_L[i][1][0] = max_x
                contour_H_L[i][1][1] = max_y
        print(f"   {contour_H_L[0][0][0]}, {contour_H_L[0][0][1]}    {contour_H_L[0][1][0]}, {contour_H_L[0][1][1]}")
        print(f"   {contour_H_L[1][0][0]}, {contour_H_L[1][0][1]}    {contour_H_L[1][1][0]}, {contour_H_L[1][1][1]}")
        # 绘制直线
        cv2.line(image_data, (contour_H_L[0][0][0],contour_H_L[0][0][1]), (contour_H_L[1][1][0], contour_H_L[1][1][1]), (255, 0, 0), 2)
        cv2.line(image_data, (contour_H_L[1][0][0],contour_H_L[1][0][1]), (contour_H_L[0][1][0], contour_H_L[0][1][1]), (255, 0, 0), 2)
        # 绘制直线), (255, 0, 0), 2)  # 蓝色直线，线宽为2
        # 在原图上绘制轮廓
        ...
        # 定义红色的HSV阈值范围
        lower_red1 = np.array([0, 100, 100])    # 红色下限1
        upper_red1 = np.array([10, 255, 255])   # 红色上限1
        lower_red2 = np.array([160, 100, 100])  # 红色下限2
        upper_red2 = np.array([180, 255, 255])  # 红色上限2
        ...
        for i, contour in enumerate(contours):
            print(f"轮廓 {i}:")
            for point in contour:
                print(f"  点坐标: {point[0][0]}, {point[0][1]}")

        # if contours is not None :
        #     for conotour in contours :
        ...
        contour_image = image_data.copy()
        cv2.drawContours(contour_image, contours, -1, (0, 255, 0), 2)  # 绿色轮廓
        cv2.imshow('red_contours.png', contour_image)  # 保存带轮廓的图像
...
def access_get_image(cam, active_way="getImagebuffer"):
    ...
    if active_way == "getImagebuffer":
        ...
        if None != stOutFrame.pBufAddr and 0 == ret and stOutFrame.stFrameInfo.enPixelType == 17301505:
                ...
                ctypes.memmove(byref(pData), stOutFrame.pBufAddr,
                                stOutFrame.stFrameInfo.nWidth * stOutFrame.stFrameInfo.nHeight)
                image = np.frombuffer(pData, count=int(stOutFrame.stFrameInfo.nWidth * stOutFrame.stFrameInfo.nHeight),
                                     dtype=np.uint8)
                image_openCv = image_control(data=image, stFrameInfo=stOutFrame.stFrameInfo)
        ...
        #openCV_dealwith(image_openCv , 1)
        ...
        if ret == 0:
                ...
                image_openCv = image_control(data=image, stFrameInfo=stFrameInfo)
        else:
                print("no data[0x%x]" % ret)
            #openCV_dealwith(image_openCv , 1)
        return image_openCv
...
def image_callback(pData, pFrameInfo, pUser):
    ...
    #打印图片信息 stFrameInfo.enPixelType
   # print("get stFrameInFo.enPixelType[%d]" % (stFrameInfo.enPixelType))
    if stFrameInfo:
        print("get one frame: Width[%d], Height[%d], nFrameNum[%d]" % (
        stFrameInfo.nWidth, stFrameInfo.nHeight, stFrameInfo.nFrameNum))
    if img_buff is None and stFrameInfo.enPixelType == 17301505:
        ...
        ctypes.memmove(byref(img_buff), pData, stFrameInfo.nWidth * stFrameInfo.nHeight)
        ...
        ctypes.memmove(byref(img_buff), pData, stFrameInfo.nWidth * stFrameInfo.nHeight * 3)
        ...
        ctypes.memmove(byref(img_buff), pData, stFrameInfo.nWidth * stFrameInfo.nHeight * 2)
